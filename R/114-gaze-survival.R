...
report_gaze_survival_model <- function(model, conf_level = .95) {
  ...
  d <- model$data
  cs <- summarise_gaze_censoring(d)[1L, , drop = FALSE]
  diagnostic <- if (grepl("^aft", model$model_family)) {
    "not applicable to AFT model"
  } else if (identical(model$backend, "coxme::coxme")) {
    "PH diagnostic not run on coxme frailty fit; report the corresponding marginal Cox diagnostic separately"
  } else {
    ph <- check_gaze_proportional_hazards(model)
    if (any(ph$ph_flag, na.rm = TRUE)) "flagged PH diagnostic" else "no PH diagnostic flag at alpha=.05"
  }
  list(
    N_participants = length(unique(d[[model$participant_col %||% "participant_id"]])),
    N_trials = nrow(d),
    N_observed_events = as.integer(cs$n_observed_events),
    N_censored_trials = as.integer(cs$n_censored),
    N_review_required = as.integer(cs$n_review_required),
    censoring_percentage = 100 * as.numeric(cs$censoring_fraction),
    event_type = sort(unique(as.character(stats::na.omit(d$event_type)))),
    target_aoi = sort(unique(as.character(stats::na.omit(d$target_aoi)))),
    time_origin = sort(unique(as.character(stats::na.omit(d$time_origin)))),
...
# `%||%` is defined in the package core; retain a local fallback for source-file
# testing and documentation builds that isolate this file.
if (!exists("%||%", mode = "function")) `%||%` <- function(x, y) if (is.null(x)) y else x

#' Generate synthetic trial windows and event rows for survival examples
#' @export
simulate_gaze_survival_inputs <- function(
  ...
  kind <- match.arg(kind)
  if (n_participants < 1L || trials_per_participant < 1L) stop("Participant and trial counts must be positive.", call. = FALSE)
  set.seed(seed)
  conditions <- if (kind == "disclosure") c("control", "minimal_disclosure", "detailed_disclosure") else c("standard", "evidence_prompt")
  shifts <- c(control = .35, minimal_disclosure = .05, detailed_disclosure = -.20, standard = .20, evidence_prompt = -.25)
  ...
  for (p in seq_len(n_participants)) {
    participant <- sprintf("P%03d", p)
    recording <- sprintf("R%03d", p)
    participant_shift <- stats::rnorm(1L, 0, .25)
    for (tr in seq_len(trials_per_participant)) {
      kt <- kt + 1L
      trial <- sprintf("%s_T%02d", participant, tr)
      condition <- conditions[((p + tr - 2L) %% length(conditions)) + 1L]
      trial_duration <- 4
      latent <- exp(.8 + shifts[[condition]] + participant_shift + stats::rnorm(1L, 0, .45))
      observed <- latent <= trial_duration
      trials[[kt]] <- data.frame(
        recording_id = recording,
        participant_id = participant,
        trial_id = trial,
        stimulus_id = sprintf("S%02d", tr),
        condition_id = condition,
        start_time = 0,
        end_time = trial_duration,
        n_valid_samples = 230L + sample(-10:10, 1L),
        valid_data_fraction = stats::runif(1L, .91, .995),
        observation_end_reason = "scheduled_trial_end",
        stringsAsFactors = FALSE
      )
      ke <- ke + 1L
      events[[ke]] <- data.frame(
        recording_id = recording,
        trial_id = trial,
        start_time = min(.35 + .04 * (tr - 1L), trial_duration - .05),
        aoi_id = "body",
  ...
  ev <- ev[order(ev$recording_id, ev$trial_id, ev$start_time, method = "radix"), , drop = FALSE]
  rownames(ev) <- NULL
  list(trials = do.call(rbind, trials), events = ev)
}
